...
// npm test -- src/controllers/ticketStatusController/tests/
...
describe('TEST updateState', () => {
  ...
  it('SUCCESS - Cambio stato da Ritirato a In lavorazione', async () => {
    ...
    // Verifica che storico_stato contenga un oggetto con stato 'Accettato'
    const hasNewStateInHistory = ticket.storico_stato.some(
      (hist) => hist.stato === IN_LAVORAZIONE
    );
    expect(hasNewStateInHistory).toBeTruthy();
  ...
  it('SUCCESS - Cambio stato da In lavorazione a Attesa conferma preventivo con nuovo prezzo', async () => {
    ...
    // Verifica che storico_stato contenga un oggetto con stato 'Accettato'
    const hasNewStateInHistory = ticket.storico_stato.some(
      (hist) => hist.stato === ATTESA_CONFERMA_PREVENTIVO
    );
    expect(hasNewStateInHistory).toBeTruthy();
  ...
  it('SUCCESS - Cambio stato da Attesa conferma preventivo a Preventivo accettato', async () => {
    ...
    // Verifica che storico_stato contenga un oggetto con stato 'Accettato'
    const hasNewStateInHistory = ticket.storico_stato.some(
      (hist) => hist.stato === PREVENTIVO_ACCETTATO
    );
    expect(hasNewStateInHistory).toBeTruthy();
  ...
  it('SUCCESS - Cambio stato da Preventivo accettato a Attesa ricambio', async () => {
    ...
    // Verifica che storico_stato contenga un oggetto con stato 'Accettato'
    const hasNewStateInHistory = ticket.storico_stato.some(
      (hist) => hist.stato === ATTESA_RICAMBIO
    );
    expect(hasNewStateInHistory).toBeTruthy();
  ...
  it('SUCCESS - Cambio stato da Completato a In consegna - Coletato', async () => {
    ...
    // Verifica che storico_stato contenga un oggetto con stato 'Accettato'
    const hasNewStateInHistory = ticket.storico_stato.some(
      (hist) => hist.stato === IN_CONSEGNA_COMPLETATO
    );
    expect(hasNewStateInHistory).toBeTruthy();
  ...
  it('SUCCESS - Cambio stato da Attesa conferma preventivo a Preventivo rifiutato', async () => {
    ...
    // Verifica che storico_stato contenga un oggetto con stato 'Accettato'
    const hasNewStateInHistory = ticket.storico_stato.some(
      (hist) => hist.stato === PREVENTIVO_RIFIUTATO
    );
    expect(hasNewStateInHistory).toBeTruthy();
  ...
  it('SUCCESS - Cambio stato da Preventivo rifiutato a In consegna rifiutato', async () => {
    ...
    // Verifica che storico_stato contenga un oggetto con stato 'Accettato'
    const hasNewStateInHistory = ticket.storico_stato.some(
      (hist) => hist.stato === IN_CONSEGNA_RIFIUTATO
    );
    expect(hasNewStateInHistory).toBeTruthy();
  ...
  it('SUCCESS - Cambio stato da Annullato a In consegna - Annullato', async () => {
    ...
    // Verifica che storico_stato contenga un oggetto con stato 'Accettato'
    const hasNewStateInHistory = ticket.storico_stato.some(
      (hist) => hist.stato === IN_CONSEGNA_ANNULLATO
    );
    expect(hasNewStateInHistory).toBeTruthy();
  ...
  // Cambio stato da In lavorazione a Attesa conferma preventivo e prezzo < 0
  it('ERRORE - Cambio stato da In lavorazione a Attesa conferma preventivo con prezzo < 0', async () => {
    mockReq.body.newstate = ATTESA_CONFERMA_PREVENTIVO;
    ticket.stato = IN_LAVORAZIONE;
    mockReq.body.prezzo = -100;
    Ticket.findById.mockResolvedValue(ticket);
  ...
  // Cambio stato da In lavorazione a Attesa conferma preventivo e ticket non trovato
  ...
  it('ERRORE - Permessi non validi per il cambio di stato', async () => {
    ...
    expect(mockNext).toHaveBeenCalledWith(expect.any(ErrorResponse));
    expect(mockNext.mock.calls[0][0].message).toContain(
      'Non sei autorizzato a cambiare stato'
    );
    expect(mockNext.mock.calls[0][0].statusCode).toBe(400);
  });

  // Tansizione di stato illegale
  it('ERRORE - Tansizione di stato illegale', async () => {
    mockReq.body.newstate = COMPLETATO;
    ticket.stato = IN_LAVORAZIONE;
    Ticket.findById.mockResolvedValue(ticket);
    ...
    expect(mockNext).toHaveBeenCalledWith(expect.any(ErrorResponse));
    expect(mockNext.mock.calls[0][0].message).toContain(
      'Transizione di stato illegale: ' +
        ticket.stato +
        ' -/-> ' +
        mockReq.body.newstate
    );
    expect(mockNext.mock.calls[0][0].statusCode).toBe(400);
  });
});
